...
5. از تب **Site & downloads** همه‌چیز قابل تغییر است (بدون دست‌کاری فایل‌ها):
   خاموش/روشن کردن دکمه دانلود عمومی و `/datasetup` + ویرایش متن‌های سایت:
   نام سایت، تیتر اصلی، زیرتیتر، متن دکمه دانلود، متن فوتر و یادداشت نسخه.
   (هر فیلد خالی بماند = متن پیش‌فرض دوزبانه.)

## 🎮 آپلود بازی (فروشگاه داخل اپ)
از تب **Game upload** در پنل:
- **روش ۱ — لینک دانلود جدا:** فقط آدرس https فایل را وارد کنید.
- **روش ۲ — آپلود مستقیم تا ۲ گیگ:** فایل zip/exe بازی را آپلود کنید؛
  SHA-256 و سایز خودکار محاسبه می‌شود. فایل‌ها در `gamedata/` (غیرعمومی)
  ذخیره و فقط از `/gamedl/<id>/<file>` با بررسی مالکیت سرو می‌شوند.
- برای هر بازی: توضیحات، قیمت (۰ = رایگان)، عکس کاور، بنر و تصویر HOVER.
- بازی‌های پولی: کاربر در اپ شماره کارت (تب Site & downloads) را می‌بیند،
  کارت‌به‌کارت پرداخت می‌کند و عکس فیش را می‌فرستد → تب **Payments** فیش را
  تأیید کنید تا مبلغ به **اعتبار** کاربر اضافه شود → کاربر بازی را می‌خرد.
  (از تب Users هم می‌توانید دستی اعتبار اضافه کنید.)
- اگر هاست اجازه آپلود ۲ گیگ نمی‌دهد، `upload_max_filesize`/`post_max_size`
  را در MultiPHP INI Editor روی 2200M بگذارید؛ در غیر این صورت از روش لینک استفاده کنید.
...
| `GET /api/store/games` · `POST /api/store/claim` `/buy` `/payment` | فروشگاه، دریافت رایگان، خرید با اعتبار، ارسال فیش |
| `GET /gamedl/<gameId>/<file>` | دانلود فایل بازی (فقط رایگان/خریداری‌شده/ادمین) |
